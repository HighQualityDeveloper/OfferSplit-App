...
public class ViewUtilsKroid {
	...
	public static View[] allChildViews(ViewGroup parent) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (child instanceof ViewGroup) {
				View[] childArr = ViewUtilsKroid.allChildViews((ViewGroup) child);
	...
	/**
	 * Does a depth first search and returns an array with itself and all child
	 * views & viewgroups.
	 **/
	public static View[] allChildViewsAndViewGroups(ViewGroup parent) {
		...
		for (int i = 0; i < childCount; i++) {
			View child = parent.getChildAt(i);
			if (child instanceof ViewGroup) {
				View[] childArr = ViewUtilsKroid
						.allChildViewsAndViewGroups((ViewGroup) child);
				arr.addAll(Arrays.asList(childArr));
			} else {
				arr.add(child);
			}
	...
	/**
	 * Mutates the state of background BitmapDrawables so that TileMode.REPEAT
	 * works properly
	 **/
	...
	public static void fixDrawableRepeat(Drawable d) {
		if (d != null) {
			if (d instanceof BitmapDrawable)
				fixBitmapRepeat((BitmapDrawable) d);
			else if (d instanceof LayerDrawable)
				fixLayerRepeat((LayerDrawable) d);
			else if (d instanceof StateListDrawable)
				fixStateListRepeat((StateListDrawable) d);
		}
	}

	/**
	 * There's a limitation where we can only get the current Drawable, so
	 * that's the only one that can be mutated
	 **/
	...
	public static void changeFontsKroid(ViewGroup root, Typeface tf) {
		...
		for (int i = 0; i < root.getChildCount(); i++) {
			...
			if (v instanceof TextView) {
				...
				changeFontsKroid((ViewGroup) v, tf);
			}
		}
	}
